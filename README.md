...
**OpenCode Termux Sandbox** — архитектурное решение и готовый набор скриптов для изолированного запуска автономного AI кодинг-агентов на смартфонах Android без root-прав.
...
Клонируйте репозиторий и запустите скрипт развертывания, с агентом OpenCode:
```bash
...
source ~/.bashrc
```
Теперь команда `o` или опенкод запускает агента из самого термукса, команда `oо` запускает безопасного агента в текущей рабочей папке!
